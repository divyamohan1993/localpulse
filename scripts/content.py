...
ABSTRACT = [
    ...
    (
        "The system combines two artificial intelligence services. The first is a "
        "social-media summarisation pipeline that ingests local Twitter or X and "
        "Reddit posts, removes duplicates and noise, classifies posts by category "
        "(road, shelter, power, water, medical) and produces a short status summary "
        "the responder can act on. The second is a multilingual voice bot built "
        "around Twilio telephony and OpenAI Whisper that lets elderly residents "
        "without smartphones call a single number and report an incident or ask "
        "for help in Hindi, Punjabi, Tamil, Bengali or English."
    ),
    (
        "The minimum lovable product is implemented in Node.js 20 with Express, a "
        "Tailwind front-end, Leaflet maps and the browser Web Speech API. It is "
        "containerised with Docker, pushed to Google Artifact Registry and served "
        "from Cloud Run in the asia-east1 region behind the custom domain "
        "localpulse.dmj.one. Cold-start latency is held under two seconds, "
        "ninety-fifth percentile request latency under two hundred milliseconds and "
        "idle cost at near zero through scale-to-zero. Future scope includes live "
        "Twitter and Reddit ingestion, Twilio production telephony, an offline-first "
        "Progressive Web Application with satellite SMS fallback, and integration "
        "with the National Disaster Management Authority data feeds."
    ),
]
...
VIVA_QA = [
    ...
    },
    {
        ...
        "a": (
            ...
            "The social-media feed is a curated mock, not live ingestion; "
            "the production design is fully specified but not yet wired "
            "to the Twitter or X and Reddit endpoints. The voice channel "
            "runs in the browser through the Web Speech API rather than "
            "on a real Twilio number, which means the demo does not "
            "exercise the actual telephony path. The data plane is in-"
            "memory and does not persist across restarts. Each limitation "
            "has a clear remedy: provision the Twitter API and Reddit "
            "credentials, buy a Twilio Indian DID and switch the webhook "
            "URL, and turn on Firestore. Beyond these, useful improvements "
            "include a satellite SMS fallback for areas where the "
            "cellular network has failed, a predictive nowcast layer "
            "trained on district rainfall and river-gauge data, and a "
            "Common Alerting Protocol bridge to the National Disaster "
